...
Reframe is not General Intelligence Company. Its design system is intentionally split across three surfaces:
...
- The two typographic tokens should be `EB Garamond` and `Instrument Serif`.
...
|------|----------------|------|
| `EB Garamond` | `--font-display` | Primary brand typography. Default for headlines, wordmarks, key body copy in written design specs, and any surface that needs gravitas without stiffness. |
| `Instrument Serif` | `--font-accent` | Contrast serif for emphasized words, italic phrases, editorial highlights, modal titles, and selective punch-up moments. |

### Current Implementation Exceptions

- `--font-display` is still backed by `Cormorant_Garamond` in code.
- Functional UI body text is still backed by `Instrument_Sans` in code.
- The trend studio title currently uses `Bungee` as an intentional loud exception.
...
- Use `Instrument Serif` for emphasis, contrast, and selective theatricality: italic fragments, featured nouns, short callouts, and highlighted phrases.
...
- Large display type is allowed here.
...
- Default written design specs to `EB Garamond` and `Instrument Serif`.
